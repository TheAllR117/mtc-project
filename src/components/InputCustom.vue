<template>
  <div class="text-left w-full h-full">
    <div :class="[`relative  ${inputHeigth}`]">
      ...
      <input
        ...
        :class="[errorInput == '' ? styleInputDefautl.toString() : styleInputError.toString()]"
        placeholder=" "
        v-model="inputValue"
        @input="handleChange"
        @focus="onFocus"
          ...
          isFocused
            ...
            : inputValue == ''
              ...
              : errorInput != ''
                ? `${bgLabel} ${errorColorText} `
                : `${bgLabel} ${colorText} `,
          `absolute ${bgLabelDark} rounded-full ${fontSize}  duration-300 transform -translate-y-4 scale-75 top-2 origin-[0] px-2 ${mxLabel} peer-focus:px-2  peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-0 `
        ]"
      >
        {{ label }}
      </label>
      ...
      <label
        v-else
        :for="id"
        :class="[`${fontSize} absolute transform -translate-y-9 px-2 mx-7 origin-center`]"
      >
        {{ label }}
      </label>
      <button
...
export default {
  ...
  setup(props, { emit }) {
    const styleInputDefautl = ref(
      `block ${props.padding} w-full h-full ${props.textDirection} ${props.fontSize} ${props.colorText} ${
        props.bgInput
      } ${props.borderRadius} ${props.borderWidth} ${props.colorBorder} touch-none appearance-none dark:${
        props.colorText
      } dark:${props.colorBorder} dark:focus:${
        props.focusColorBorder
      } focus:outline-none focus:ring-0 ${props.focusColorBorder!} peer`
    )
    const styleInputError = ref(
      `block px-10 py-3 w-full h-full ${props.textDirection} ${props.fontSize} ${props.colorText} ${
        props.bgInput
      } ${props.borderRadius} ${props.borderWidth} ${props.errorColorBorder} touch-none appearance-none dark:${
        props.colorText
      } dark:${props.errorColorBorder} dark:focus:${
        props.errorColorBorder
