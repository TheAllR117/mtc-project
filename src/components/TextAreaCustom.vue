      ...
      <label
        ...
        :class="[
          isFocused
            ...
            ? `${bgLabel} ${errorColorText} `
            : `${bgLabel} ${colorText} `,
          `absolute rounded-full text-sm duration-300 transform -translate-y-4 scale-75 top-2 origin-[0] ${bgLabelDark} px-2 mx-7 peer-focus:px-2 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-0`
        ]"
      >
        {{ label }}
      </label>
...
export default {
  ...
  setup(props, { emit }) {
    const styleInputDefautl = ref(
      `block px-10 py-3 w-full h-full text-sm ${props.colorText} ${props.bgInput} ${
        props.borderRadius
      } ${props.borderWidth} ${props.colorBorder} appearance-none dark:${props.colorText} dark:${
        props.colorBorder
      } dark:focus:${
    ...
    const styleInputError = ref(
      `block px-10 py-3 w-full h-full text-sm ${props.colorText} ${props.bgInput} ${
        props.borderRadius
      } ${props.borderWidth} ${props.errorColorBorder} appearance-none dark:${
        props.colorText
